...
def download_dataset():
    ...
    print(f"Dataset 'ai4bharat/samanantar' downloaded!")
    return dataset
...
def build_tokenizer(dataset, ext: str, tkizer_save_path: str, tkizer_to_save: bool = False, vocab_size: int = 50000) -> MarianTokenizer:
    ...
    text_iterator = (item[ext] for item in dataset)
    ...
    custom_tokenizer.add_special_tokens({'unk_token': '<unk>'})
    print(f"Tokenizer for '{ext}' built")

    if tkizer_to_save:
        if not os.path.exists(tkizer_save_path):
            os.makedirs(tkizer_save_path)
        custom_tokenizer.save_pretrained(tkizer_save_path)
        print(f"Tokenizer for '{ext}' saved")

    return custom_tokenizer

# -> Dict[str, DataLoader]
def prepare_dataloaders(dataset_splits, src_tokenizer, tgt_tokenizer, batch_size):
    dataloaders = {}
    for split, data in dataset_splits.items():
        custom_dataset = TokenizedDataset(data, src_tokenizer, tgt_tokenizer)
        dataloaders[split] = DataLoader(custom_dataset, batch_size=batch_size, shuffle=(split == 'train'))
